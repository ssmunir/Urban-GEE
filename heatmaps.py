...
def generate_population_heatmap(file_path, output_folder=heatmaps):
    ...
    bin_labels_upper = [str(i) for i in range(0, 31000, 1000)]
    # Assign labels dynamically: "0", "(0,1]", "(1,2]", ..., "(29,30]"
    bin_labels =["0"] + [f"({i},{i+1}]" for i in range(0, 30)] + [">30"]
    ...
    ax = sns.heatmap(
        heatmap_data, cmap="Blues", cbar_kws={'label': 'Population Share (log)'}, square=True,  norm=LogNorm()
    )
