...
var createUrbanLayer = function(image, year) {
  var urbanLayer = image.eq(21).or(image.eq(22)).or(image.eq(23)).or(image.eq(30))
      .multiply(1) // Ensure it's 1 for urban areas
      .where(image.neq(21).and(image.neq(22)).and(image.neq(23)).and(image.neq(30)), 0); // Set non-urban areas to 0
...
var popChange_2000_2020 = pop2020.subtract(pop2000);

// Calculate the change in urban population between 2010 and 2020
var popChange_2010_2020 = pop2020.subtract(pop2010);

// Calculate the change in urban population between 2010 and 2030
var popChange_2010_2030 = pop2030.subtract(pop2010);

// Calculate the change in urban population between 2020 and 2030
var popChange_2020_2030 = pop2030.subtract(pop2020);
...
// 3) DEFAULT CRS & GROUP OVERRIDES
var defaultCRS = 'EPSG:4326';
// Each entry groups multiple countries under one CRS
var crsGroups = [
  { crs: 'EPSG:3857', countries: ['Nigeria', 'Ghana', 'Swaziland', 'Benin', 'Burkina Faso', 'Cape Verde', 'Cameroon', 
...
function computeZonalStatsAndExport(raster, year, rasterName, fileNamePrefix) {
  ...
  crsGroups.forEach(function(group) {
    ...
    var mapped = stats.map(function(f) {
      return ee.Feature(null)
        .set('country',    f.get('ADM0_NAME'))
        .set(rasterName,   f.get('sum'))
        .set('crs_used',   crs);
    });
  ...
  var mappedDef = statsDef.map(function(f) {
    return ee.Feature(null)
      .set('country',    f.get('ADM0_NAME'))
      .set(rasterName,   f.get('sum'))
      .set('crs_used',   defaultCRS);
  });
  ...
  Export.table.toDrive({
    ...
    fileFormat:  'CSV',
    selectors:   ['country', rasterName, 'crs_used']
  });
}
...
rasters.forEach(function(r) {
  ...
  computeZonalStatsAndExport(
    r.image.updateMask(r.image.gt(0)),
    r.year,
    rasterName,
    r.prefix
  );
